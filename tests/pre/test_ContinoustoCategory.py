...
def test_ContinuoustoCategory_transform_City_ignore_integer(df_City):
    ...
    assert (
        (g.transform(dfi, integer=False) == dfi)
        .all()
        .all()
    )
# 4b
def test_ContinuoustoCategory_transform_City_ignore_integer_inplace_True(df_City):
    g = ContinuoustoCategory()
    dfi = df_City.astype("int")
    assert ((g.transform(dfi, integer=False,inplace=True) == dfi)
            .all()
            .all()
    )
    # 4c
def test_ContinuoustoCategory_transform_City_ignore_integer_inplace_Drop_True(df_City):
    ...
    assert (
        (g.transform(dfi, integer=False, inplace=True, drop=True) == dfi).all().all())
    # 5
def test_ContinuoustoCategory_transform_City_integer(df_City):
    ...
    CityC[['NOX']] = CityC[['NOX']] * 100  # (parts per 1 billion)
    dfi = CityC.astype("int")
    assert(g.transform(dfi,inplace=True,drop=True) == dfi.astype("category")).all().all()
    # 5b
def test_ContinuoustoCategory_transform_City_integer_inplace_True(df_City):
    ...
    CityC[['NOX']] = CityC[['NOX']] * 100  # (parts per 1 billion)
    dfi = CityC.astype("int")
    assert(g.transform(dfi,inplace=True,drop=False) == dfi.astype("category")).all().all()
    #5c
...
def test_ContinuoustoCategory_transform_City_integer_column_names(df_City):
    ...
    CityC[['NOX']] = CityC[['NOX']] * 100  # (parts per 1 billion)
    dfi = CityC.astype("int")
    assert (g.transform(dfi,inplace=True).columns == dfi.astype("category").columns).all()
# 6b
def test_ContinuoustoCategory_transform_City_integer_column_names_inplace_def_false(df_City):
    g = ContinuoustoCategory()
    CityC =df_City.copy()
    CityC[['NOX']] = CityC[['NOX']] * 100  # (parts per 1 billion)
    dfi = CityC.astype("int")
...
def test_ContinuoustoCategory_transform_Xobect_ignore_object(Xobject):
    g = ContinuoustoCategory()
    assert (
        (g.transform(Xobject, integer=False,inplace=True) == Xobject)
        .all()
        .all()
    )
# 8
def test_ContinuoustoCategory_transform_Xobject_not_datetime_format(Xobject):
    g = ContinuoustoCategory()
    assert (g.transform(Xobject,inplace=True) == Xobject.astype("category")).all().all()
# 9
def test_ContinuoustoCategory_transform_Xboolean_ignore_object(Xboolean):
    g = ContinuoustoCategory()
    assert (
        (g.transform(Xboolean, integer=False) == Xboolean)
        .all()
        .all()
    )
# 10
def test_ContinuoustoCategory_transform_Xboolean(Xboolean):
    g = ContinuoustoCategory()
    assert (g.transform(Xboolean,inplace=True) == Xboolean.astype("category")).all().all()
...
# 11
def test_ContinuoustoCategory_transform_df_internet_traffic_ignore_object_and_integer(df_internet_traffic):
    g = ContinuoustoCategory()
    assert (        (
            g.transform(
                df_internet_traffic,inplace=True
            )
            == df_internet_traffic
        )
        .all()
...
def test_ContinuoustoCategory_transform_df_internet_traffic_ignore_datetime_and_integer(
    ...
    assert (
        (
            g.transform(
                df_City, integer=False,inplace=True
            )
            == df_City
        )
        .all()
